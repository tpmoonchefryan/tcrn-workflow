...
`상태: 0.1.0-rc.4 (프리릴리스 후보)` · `라이선스: Apache-2.0` · `Node 24.16.0` · `pnpm 11.3.0` · `검증된 클레임: 64`
...
| **기계가 읽는 클레임 원장** | `verification-map.yaml`이 64개의 기능 클레임을 관측 가능한 사유 코드에 결속합니다. 클레임의 대상이 바뀌면 증명을 다시 실행해야 합니다 — 과대 주장은 스타일 문제가 아니라 빌드 실패입니다. |
...
- `verification-map.yaml`의 **기계 검증 클레임 64개**.

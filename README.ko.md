...
`상태: 0.1.0-rc.4 (프리릴리스 후보)` · `라이선스: Apache-2.0` · `Node 24.16.0` · `pnpm 11.3.0` · `검증된 클레임: 34`
...
| **페일 클로즈드 검증 체인** | 명령 하나(`pnpm verify:p1`)로 20개의 게이트 실행: 포맷, 린트, 타입 검사, 빌드, 약 29개 테스트 파일, 신뢰 매트릭스, 아카이브/SBOM/라이선스/취약점 정책, 소스 허용 목록, 오프라인 경계, 프라이버시 스캔, CI 강화, 검증 맵, 클린 히스토리 증명. 예상 밖의 것은 체인을 멈춥니다. |
| **기계가 읽는 클레임 원장** | `verification-map.yaml`이 34개의 기능 클레임을 관측 가능한 사유 코드에 결속합니다. 클레임의 대상이 바뀌면 증명을 다시 실행해야 합니다 — 과대 주장은 스타일 문제가 아니라 빌드 실패입니다. |
| **듀얼 호스트 Agent App 어댑터** | Codex와 Claude Code가 V1 공식 지원 호스트입니다. 바이트 단위로 동일한 호스트 중립 기계를 공유하며 크로스 호스트 패리티 다이제스트로 증명됩니다. 두 어댑터 모두 **비활성 드라이런 후보**로, 설치되지 않은 템플릿 데이터만 생성하며 라이브 호스트 지원은 일절 주장하지 않습니다. |
...
3. **거버넌스에는 직렬화 가능한 서사가 필요합니다.** 단일 작성자 체인은 결정의 선형적이고 변조 감지 가능한 *순서*를 제공합니다. 각 결정을 책임 있는 행위자와 그 증거에 묶는 일은 오늘은 거버넌스 스레드의 영수증이 담당하며, 활성화되면 워크스페이스 체인 내 행위자 증명 확장이 담당합니다. 공유 상태를 서로 고치는 피어 스레드 무리에는 순서도 결속도 없습니다.
...
- 엔진, 지식 코어, 아티팩트 라이프사이클, 프로파일, 페르소나, 컨텍스트 라우터, 두 어댑터, 교환, 호환, 요구사항 원장, 릴리스 후보, 프라이버시 경계, 증명 아티팩트 생성기, 신뢰 매트릭스를 덮는 **약 29개 테스트 파일**.
- `verification-map.yaml`의 **기계 검증 클레임 34개**.
...
- `0.1.0-rc.4`는 **프리릴리스 후보**입니다. 공개 API는 아직 안정적이지 않습니다.

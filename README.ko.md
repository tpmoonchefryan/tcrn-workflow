...
| | |
| --- | --- |
| ✅ **맞습니다** | 결과가 따르는 일에 에이전트를 쓰는 경우 — 프로덕션 코드, 규제 또는 감사 대상 딜리버리, 누가 무엇을 결정했는지 아무도 기억하지 못하는 멀티 에이전트 인수인계. 리뷰어가 *믿어야만* 하는 대화 기록이 아니라 *검증할 수 있는* 산출물을 원하는 경우. 그리고 모든 것을 자기 머신 안에 두고 싶은 경우: 데이터베이스 없음, 데몬 없음, 네트워크 없음, 텔레메트리 없음. 또한 당신의 에이전트가 엄격한 규율을 따를 수 있을 만큼 프런티어급인 경우 — 「알려진 한계」를 보십시오. |
...
| **기계가 읽는 주장 원장** | `verification-map.yaml`이 69개 주장 — framework-hygiene 13개, inertness-proof 13개, runtime-capability 43개 — 을 관측 가능한 reason code에 결속합니다. 주장의 주어가 바뀌면 그 증명은 다시 실행되어야 합니다. |
...
위의 빠른 시작이 증명하는 것은 프레임워크 자체입니다. *사용*은 다른 활동이며 — 의도적으로, 명령어를 타이핑하는 일이 아닙니다.

**거버넌스 루프 전체를 한 번 직접 걸어 보고 싶다면** — 워크스페이스 → 이니셔티브 → 에픽 → 스토리 → 게이트 → 콘퍼런스 → 증류된 지식 → 추적 — [튜토리얼](docs/tutorial/governed-loop.md)을 따라 하십시오. 튜토리얼의 모든 명령은 `pnpm verify:e2e`가 그대로 실행하므로 조용히 썩을 수 없습니다.

**실제 작업에서는 에이전트가 기록하고, 당신이 결정합니다.** 의도된 조작자는 AI 에이전트 — Claude Code 또는 Codex — 이며, 동반 프로젝트 **tcrn-workflow-helper**(이 저장소와 나란히 공개됨)의 Skill을 에이전트의 skills 폴더에 배치합니다. 이 Skill이 운영 규율을 담고 있습니다: 모든 단계를 쉬운 말로 설명하는 최초 실행 마법사, 작업의 순간을 그것을 기록하는 동사로 연결하는 라우팅 지침, 그리고 하나의 단단한 규칙이 관통하는 기록 규율 — **당신의 명시적 동의 없이는 아무것도 기록되지 않습니다**.

그 이후의 작업 세션은 이렇습니다:

1. **평소처럼 에이전트와 방향을 논의합니다.** 대화가 결과를 낳는 무언가 — 결정, 분해, 완료된 산출물 — 를 만들면, 에이전트는 무엇을 어떤 동사로 기록할지 명시하며 기록을 *제안*합니다. 당신의 "예"가 기록하고, "아니오"면 그대로 사라집니다.
2. **다툼이 있는 "완료"에는 게이트가 섭니다.** 충족되지 않은 게이트는 전환을 거부합니다 — 명령에서 한 번, 리플레이에서 다시 한 번 — 닫힌 콘퍼런스 회의록을 인용해 충족될 때까지. `owner_intent_required` 게이트는 대역 외 명부가 허용하지 않는 행위자를 추가로 거부합니다.
3. **숙의는 콘퍼런스이며, 이름 있는 역할로서 논의됩니다.** 엔진은 출고 시 8개의 비활성 *Core Reference 페르소나*를 담고 있습니다 — 다이제스트에 묶인 역할 명부로, 각각 사명·권한 경계·명시적 거부 항목을 가집니다: **Minerva**(워크플로 아키텍처), **Verity**(검증), **Sable**(보안 및 프라이버시), **Janus**(수용 판정), **Ilya**(구현), **Mara**(제품), **Mneme**(지식 관리), **Arturo**(오케스트레이션). 이들은 **참조 데이터이지 실행 중인 에이전트가 아닙니다**. 당신의 구동 에이전트는 그들**로서** 논쟁합니다: 다툼이 있는 질문은 mandate가 실제로 충돌하는 역할들에게 팬아웃되고, 각 입장은 모델 이름이 아니라 그 **역할** 아래 그대로 기재됩니다 — 그래서 기록은 어떤 mandate가 무엇을 주장했고 왜 소통해야 했는지를 보여줍니다. 회의록이 숙의를 판정하고, 닫힌 결정은 큐레이션된 지식으로 증류될 수 있습니다.
4. **세션과 세션 사이, 기록이 곧 기억입니다.** `status`와 각 list 동사가 그것을 읽어 오고, `work-show`는 각 항목의 권위 있는 범위와 그것을 결정한 회의록을 담으며, 스냅숏이 당신이 정한 주기로 체인을 보호합니다.

당신은 언제나 의사결정자이고, 엔진은 결정된 것을 강제하며, 체인이 증명입니다. 규율 수준에 못 미치는 에이전트는 이유 코드 위에서 헛돌 뿐 아무것도 더럽히지 못합니다 — "알려진 한계"가 규율의 정확한 요건을 적고 있습니다.
...
- `verification-map.yaml`의 **기계 검증 주장 69개** — framework-hygiene 13개, inertness-proof 13개, runtime-capability 43개. 위의 주장 배지는 실행할 때마다 파싱되어 원장과 대조됩니다.
...
위의 네 경계는 영구적인 설계 결정입니다. 아래는 이 릴리스의 운영상 사실입니다: 각 항목은 reason code로 강제되거나, 측정으로 고정되었거나, 검증되지 않은 영역으로 정직하게 기재되어 있습니다.
...
- **복원은 동일 경로만 가능합니다**. 다섯 루트의 정체성은 init 시 고정되고 매 resolve마다 재검증됩니다(`WORKSPACE_SCHEMA_INVALID`). 다른 경로나 다른 머신으로의 복원은 V1 범위 밖입니다(`WORKSPACE_MIGRATION_APPLY_UNAVAILABLE`). 백업은 어디로든, 복원은 제자리로.
- **제어 트리 전체를 복원하거나 아무것도 하지 마십시오**. 지식 저장소와 아티팩트 저장소는 이벤트 체인의 하이워터 다이제스트에 묶여 있어, 단독으로 복원된 저장소는 벽돌이 됩니다(`KNOWLEDGE_HIGH_WATER_MISMATCH`).
- **git은 무결성의 증인이지 복원 도구가 아닙니다**. 워크스페이스 루트에 문서화된 ignore 목록과 함께 저장소를 두면 두 번째 증인을 얻습니다. 실제 복원은 스냅샷 매니페스트를 거칩니다. git은 저장소들이 요구하는 빈 디렉터리를 재생성할 수 없기 때문입니다.
- **워크스페이스 사이에서 저장소 파일을 복사하지 마십시오**. 각 저장소는 자기 워크스페이스의 히스토리에 묶여 있습니다. 워크스페이스 간 이동은 오늘 계획 표면입니다: `exchange-plan`·`exchange-dry-run`·`exchange-validate`는 존재하고, 적용하는 동사는 존재하지 않습니다.

**검증된 범위**

- **단일 OS 사용자, 로컬 파일시스템**. 모든 테스트와 모든 실기 관측이 이 범위에서 이루어졌습니다. 사용자 간 공유와 네트워크 파일시스템은 검증되지 않았으므로 주장하지 않습니다.

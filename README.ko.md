...
| **되돌릴 수 있는 활성화** | 명시적이고 바이트 단위로 가역적인 세 단계가 비활성 Claude Code 번들을 살아 있는 통제 세션으로 바꿉니다 — 그리고 세션 훅의 어떤 오류든 평범한 Claude Code로 깨끗하게 빠져나갑니다. 비활성 번들이 `~/.claude` 아래의 무언가를 지목하거나 쓰는 일은 결코 없습니다. |
| **스스로를 증명하는 백업** | 스냅샷은 결정적인 파일별 매니페스트를 냅니다. 런북은 스냅샷 → 삭제 → 복원을 바이트 단위로 왕복시키고, 정말 중요한 두 실패 모드(부분 복원, 다른 위치 복원)는 페일클로즈합니다. |
| **두 호스트, 하나의 진실** | Codex와 Claude Code 어댑터는 바이트 단위로 동일한 호스트 중립 기계를 공유하며, 호스트 간 일치 다이제스트로 증명됩니다. 둘 다 기본적으로 **비활성 드라이런 후보**이며, 설치되지 않은 템플릿 데이터만 생성합니다. |
...
맨 아래에 동결된 프로토콜, 그 위에 파일 네이티브 엔진, 다시 그 위에 역량 계층, 맨 위에 비활성 호스트 어댑터. 프로토콜은 추가 전용입니다: `work-model-v1`은 동결되었고, 모든 확장은 이미 수용된 스키마를 건드리지 않고 스스로 등록합니다.
...
### Codex와 Claude Code 어댑터는 왜 "비활성 후보"인가

통제된 릴리스 경로가 수용하기 전에 라이브 호스트 지원을 주장하는 것은 과잉 주장이며 — 바로 이 프레임워크가 막기 위해 존재하는 실패 모드이기 때문입니다. 어댑터는 결정적이고 설치되지 않은 템플릿 번들을 생성합니다(바이트 단위로 정확함이 증명되었고, 사용자 내용을 결코 덮어쓰지 않는 바이트 가역 `.claude/settings.json` 훅 조각을 포함합니다). 활성화는 별개의, 게이트가 걸린 결정입니다.
...
- 두 호스트 어댑터 모두 비활성 드라이런 후보이며, **라이브 Codex 또는 Claude Code 지원은 주장하지 않습니다**.

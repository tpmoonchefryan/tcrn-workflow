...
- **활성화에는 아직 사용자용 명령 경로가 없습니다**. 위 단계들은 증거 하네스가 구동했으며, 그것이 통치되는 호스트 컨텍스트를 프로그램적으로 공급합니다. 배포되는 CLI는 공급하지 않으므로 셸에서 `claude-adapter-install`을 실행하면 `ADAPTER_HOST_REQUIRED`에서 멈춥니다. 역량과 그 영수증은 실재합니다. 만들어지지 않은 것은 운영자 진입점입니다. 이후 릴리스에서 추가됩니다.

...
> [!TIP]
> **You do not have to trust this README**. Install it and run one command: it proves all 7 of its own claims to you, fully offline.
...
| :--- | :--- | :--- |
| **Did the tests actually run** | One line in a chat window | `pnpm verify:p1` — 15 gates in sequence, anything unexpected stops it |
...
That standard covers all **7 claims**. Each one is bound in `verification-map.yaml` to a stable reason code, to a proof that runs offline, and to a red leg — a statement of what change makes it go red, with that failure actually observed. All 7, with no exceptions.

<details>
<summary><b>How the 7 criteria break down</b></summary>

<br>

| Category | Count | What it covers |
| :--- | ---: | :--- |
| `framework-hygiene` | 7 | The framework's own hygiene: clean history, source allowlist, licence and vulnerability policy, offline boundary |
| `inertness-proof` | 0 | Inertness: a host adapter does nothing at all after install until someone explicitly approves activation |
| `runtime-capability` | 0 | Runtime capability: event chain, lease, views, knowledge core, context router, release set |
...
| **15 gates in one command** | `pnpm verify:p1` runs format, lint, types, build, 119 test files, the trust matrix, archive and SBOM and licence and vulnerability policy, source allowlist, offline boundary, privacy scan, CI hardening, the claim ledger and clean history. |
| **7 machine-readable criteria** | 7 framework-hygiene, 0 inertness-proof, 0 runtime-capability. All carry red legs, all bound to observable reason codes. |
...
# 2. Watch the framework prove itself: 15 gates, fully offline
pnpm verify:p1

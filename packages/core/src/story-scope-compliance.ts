...
export interface VerificationClaimLink {
  readonly id: string;
  readonly workId?: string | null;
  readonly gwt?: readonly string[];
}

export interface StoryVerificationLinkProblem {
  readonly code: "STORY_VERIFICATION_CLAIM_MISSING" | "STORY_VERIFICATION_GWT_MISSING";
  readonly message: string;
}

export interface StoryVerificationLinkValidation {
  readonly ok: boolean;
  readonly claims: readonly VerificationClaimLink[];
  readonly problems: readonly StoryVerificationLinkProblem[];
}

function workReferenceKeys(work: Pick<WorkRecord, "id" | "externalKey">): readonly string[] {
  return [work.id, work.externalKey].filter((value): value is string => typeof value === "string" && value.length > 0);
}

export function verificationClaimsForWork(
  claims: readonly VerificationClaimLink[],
  work: Pick<WorkRecord, "id" | "externalKey">,
): readonly VerificationClaimLink[] {
  const keys = new Set(workReferenceKeys(work));
  return claims
    .filter((claim) => typeof claim?.workId === "string" && keys.has(claim.workId))
    .slice()
    .sort((left, right) => left.id < right.id ? -1 : left.id > right.id ? 1 : 0);
}

export function verificationWorksForClaim(
  claim: VerificationClaimLink,
  works: readonly WorkRecord[],
): readonly WorkRecord[] {
  const workId = claim?.workId;
  if (typeof workId !== "string") return [];
  return works.filter((work) => workReferenceKeys(work).includes(workId));
}

/**
 * A normalized record with a null scopeDigest is a pre-link historical record
 * and remains readable; a current scoped Story reaching done must have a map
 * claim and GWT reference.
 */
export function validateStoryVerificationLinks(
  story: Pick<WorkRecord, "kind" | "status" | "tombstone" | "scopeDigest" | "id" | "externalKey">,
  claims: readonly VerificationClaimLink[] = [],
): StoryVerificationLinkValidation {
  const linked = verificationClaimsForWork(claims, story);
  const problems: StoryVerificationLinkProblem[] = [];
  if (story.kind !== "Story" || story.tombstone || story.status !== "done" || typeof story.scopeDigest !== "string") {
    return { ok: true, claims: linked, problems };
  }
  if (linked.length === 0) {
    problems.push({ code: "STORY_VERIFICATION_CLAIM_MISSING", message: `Story ${story.externalKey} has no verification-map claim` });
  }
  for (const claim of linked) {
    if (!Array.isArray(claim.gwt) || claim.gwt.length === 0 || claim.gwt.some((entry) => typeof entry !== "string" || entry.length === 0)) {
      problems.push({ code: "STORY_VERIFICATION_GWT_MISSING", message: `verification claim ${claim.id} names no GWT for Story ${story.externalKey}` });
    }
  }
  return { ok: problems.length === 0, claims: linked, problems };
}

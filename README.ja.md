...
[なぜこのプロジェクトが存在するのか](#なぜこのプロジェクトが存在するのか) · [あなたに向いているか](#あなたに向いているか) · [得られるもの](#得られるもの) · [クイックスタート](#クイックスタート) · [率直な回答](#率直な回答) · [ライセンス](#ライセンス)
...
| | |
| --- | --- |
| ✅ **向いています** | 結果を伴う仕事——本番コード、規制対象または監査対象のデリバリー、誰が何を決めたか誰も覚えていないマルチエージェントの引き継ぎ——にエージェントを使っている場合。レビュアーが*信じる*しかない会話ログではなく、*検証できる*成果物が欲しい場合。そして、すべてを自分のマシン内に留めたい場合：データベースなし、デーモンなし、ネットワークなし、テレメトリなし。 |
...
| **取り消せるアクティベーション** | 三つの明示的でバイト単位に可逆な手順が、不活性な Claude Code バンドルをライブな統制セッションに変えます——そしてセッションフックのいかなるエラーも、素の Claude Code としてクリーンに終了します。不活性バンドルが `~/.claude` 配下を名指ししたり書き込んだりすることは決してありません。 |
...
| **二つのホスト、一つの真実** | Codex と Claude Code のアダプターは、バイト単位で同一のホスト中立機構を共有し、ホスト間一致 digest によって証明されています。どちらも既定では**不活性なドライラン候補**であり、未インストールのテンプレートデータのみを生成します。 |
...
最下層に凍結されたプロトコル、その上にファイルネイティブなエンジン、さらに上に能力レイヤー、最上部に不活性なホストアダプター。プロトコルは追記のみです：`work-model-v1` は凍結され、すべての拡張は受理済みスキーマに触れずに自己登録します。
...
### なぜ Codex と Claude Code のアダプターは「不活性な候補」なのか

統制されたリリース経路が受理する前にライブホスト対応を主張することは過大宣言であり——まさにこのフレームワークが防ぐために存在する失敗モードだからです。アダプターは決定的で未インストールのテンプレートバンドルを生成します（バイト単位で厳密に証明済み。ユーザー内容を決して壊さない、バイト単位で可逆な `.claude/settings.json` フックフラグメントを含みます）。アクティベーションは別個の、ゲート付きの決定です。
...
- 両ホストアダプターは不活性なドライラン候補であり、**ライブの Codex / Claude Code 対応は主張しません**。

...
| **機械が読めるクレーム台帳** | `verification-map.yaml` が 68 のクレーム——13 の framework-hygiene、13 の inertness-proof、42 の runtime-capability——を観測可能な reason code に束縛します。クレームの主語が変われば、その証明は再実行されなければなりません。 |
...
- **68 の機械検証済みクレーム**が `verification-map.yaml` にあります——13 の framework-hygiene、13 の inertness-proof、42 の runtime-capability。上部のクレームバッジは毎回解析され、台帳と照合されます。

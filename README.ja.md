...
3. **ガバナンスには直列化可能な物語が必要。**単一ライターのチェーンは決定の線形かつ改竄検知可能な*順序*を与えます。各決定を説明責任のある実行主体とその証拠へ結び付けることは、今日はガバナンススレッドのレシートが担い、有効化された際にはワークスペースチェーン内の実行主体アテステーション拡張が担います。共有状態を書き換え合うピアスレッドの群れには、順序も結び付けもありません。

...
| **機械が読めるクレーム台帳** | `verification-map.yaml` が 75 のクレーム——13 の framework-hygiene、13 の inertness-proof、49 の runtime-capability——を観測可能な reason code に束縛します。クレームの主語が変われば、その証明は再実行されなければなりません。 |
| **効き目が残っていることを自ら示すガード** | `pnpm guard-check` は登録済みの各ガードをソースから変異除去し、指定されたテストが赤になることを要求します——18 のガードを、プッシュのたびに検証。失われても誰も気づかない保護は、保護ではありません。 |
...
- **75 の機械検証済みクレーム**が `verification-map.yaml` にあります——13 の framework-hygiene、13 の inertness-proof、49 の runtime-capability。上部のクレームバッジは毎回解析され、台帳と照合されます。
- **18 の登録済みガード**。それぞれ変異除去してテストが赤になることを確認し、今も効いていることを証明しています。
...
| `verify:act4` / `verify:act9` / `verify:act10` / `verify:act11` | Codex inert install、独立に認可された単一 SessionStart activation と exact-definition drift、read-only supplied-stream の subagent/thread/turn collection、明示的な unavailable セルを保持するクロスホスト acceptance/hostile matrix。 |

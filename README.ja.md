...
| **機械が読めるクレーム台帳** | `verification-map.yaml` が 76 のクレーム——13 の framework-hygiene、13 の inertness-proof、50 の runtime-capability——を観測可能な reason code に束縛します。クレームの主語が変われば、その証明は再実行されなければなりません。 |
| **効き目が残っていることを自ら示すガード** | `pnpm guard-check` は登録済みの各ガードをソースから変異除去し、指定されたテストが赤になることを要求します——20 のガードを、プッシュのたびに検証。失われても誰も気づかない保護は、保護ではありません。 |
...
上のクイックスタートが証明するのはフレームワークそのものです。*使う*ことは別の営みであり——意図的に、コマンドを打つことではありません。

**ガバナンスされたループを一度、手で最後まで歩きたいなら**——ワークスペース → イニシアチブ → エピック → ストーリー → ゲート → カンファレンス → 蒸留された知識 → トレース——[チュートリアル](docs/tutorial/governed-loop.md)に従ってください。チュートリアル内のすべてのコマンドは `pnpm verify:e2e` によって逐語的に実行されるため、静かに腐ることができません。

**実際の仕事では、エージェントが筆を執り、あなたが裁決します。** 想定される操作者は AI エージェント——Claude Code または Codex——であり、コンパニオンの **tcrn-workflow-helper** Skill（本リポジトリと並んで公開） をその skills フォルダに配置します。この Skill が運用規律を担います：各ステップを平易な言葉で説明する初回セットアップウィザード、仕事の瞬間をそれを記録する動詞へ対応付けるルーティング指針、そして一つの硬い規則が貫く記録規律——**あなたの明示的な同意なしには、何も書き込まれません**。

その後の作業セッションはこうなります：

1. **いつも通りエージェントと方向性を議論します。** 会話が帰結を持つ何か——決定、分解、完了した成果物——を生んだとき、エージェントは記録することを*提案*し、何をどの動詞で記録するかを名指しします。あなたの「はい」が書き込み、「いいえ」で流れます。
2. **争いのある「完了」にはゲートが立ちます。** 未充足のゲートは遷移を拒否します——コマンド時に一度、リプレイ時にもう一度——閉じたカンファレンス議事録を引用して充足されるまで。`owner_intent_required` のゲートはさらに、帯域外の名簿が許可しないアクターをすべて拒否します。
3. **熟議はカンファレンスであり、名前のある役割として論じられます。** エンジンは出荷時に 8 つの不活性な *Core Reference ペルソナ*を備えます——ダイジェストに束縛された役割の名簿で、それぞれに使命・権限境界・明示的な拒否事項があります：**Minerva**（ワークフロー設計）、**Verity**（検証）、**Sable**（セキュリティとプライバシー）、**Janus**（受け入れ判定）、**Ilya**（実装）、**Mara**（プロダクト）、**Mneme**（知識統治）、**Arturo**（オーケストレーション）。これらは**参照データであり、稼働中のエージェントではありません**。あなたの駆動エージェントはそれらの役割**として**論じます：争いのある問いは、mandate が実際に衝突する役割へ扇状に分配され、各立場はモデル名ではなくその**役割**のもとで逐語的に記帳されます——だから記録は、どの mandate が何を主張し、なぜ話し合う必要があったかを示します。議事録が熟議を裁定し、閉じた決定はキュレーションされた知識へ蒸留できます。
4. **セッションとセッションの間、記録が記憶です。** `status` と各 list 動詞がそれを読み戻し、`work-show` は各項目の権威あるスコープとそれを裁定した議事録を携え、スナップショットがあなたの選んだ節目でチェーンを守ります。

あなたは常に意思決定者であり、エンジンは決定されたことを強制し、チェーンが証明です。規律の水準に満たないエージェントは理由コードの上で空転するだけで、何も汚せません——「既知の限界」が規律の正確な要件を記しています。
...
- **76 の機械検証済みクレーム**が `verification-map.yaml` にあります——13 の framework-hygiene、13 の inertness-proof、50 の runtime-capability。上部のクレームバッジは毎回解析され、台帳と照合されます。
- **20 の登録済みガード**。それぞれ変異除去してテストが赤になることを確認し、今も効いていることを証明しています。
...
| `verify:act4` / `verify:act9` / `verify:act10` / `verify:act11` / `verify:act12` | Codex inert install、独立認可された SessionStart activation と definition drift、read-only supplied-stream collection、cross-host matrix、hostile cwd から Codex/Claude command が admitted absolute root だけを実行する証明。 |

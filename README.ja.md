...
`ステータス: 0.1.0-rc.5(プレリリース候補)` · `ライセンス: Apache-2.0` · `Node 24.16.0` · `pnpm 11.3.0` · `検証済みクレーム: 65`
...
| **機械可読なクレーム台帳** | `verification-map.yaml` が 65 の機能クレームを観測可能な理由コードに束縛。クレームの対象が変われば証明の再実行が必須——過大宣言はスタイルの問題ではなくビルド失敗です。 |
...
- `verification-map.yaml` の **65 の機械検証クレーム**。

...
- **アクティベーションには、まだ利用者向けのコマンド経路がありません**。上記の各ステップは証跡ハーネスによって駆動されており、統治されたホストコンテキストをプログラム的に供給しています。出荷される CLI はそれを供給しないため、シェルから `claude-adapter-install` を実行すると `ADAPTER_HOST_REQUIRED` で止まります。能力もそのレシートも実在します。作られていないのは操作者向けの入口です。後続リリースで追加します。
...
- **スケール**。ワークスペースは**数千件台前半**のイベント数で体感できるほど遅くなり、単一コマンドが一秒を超えるのはおよそ **6,600** 件です（Apple M3 上の実測からの外挿。`docs/verification/2026-07-20-event-chain-ceiling-samples.json`）。これは書き込みと同じだけ読み取りにも効きます。組織全体を一本のチェーンで走らせるのではなく、**プロジェクトまたはイニシアチブ単位でワークスペースを分割**してください。

...
`状态: 0.1.0-rc.4(预发布候选)` · `许可证: Apache-2.0` · `Node 24.16.0` · `pnpm 11.3.0` · `已验证声明: 34`
...
| **失败即关闭的验证链** | 一条命令(`pnpm verify:p1`)跑 20 道门:格式、lint、类型检查、构建、约 29 个测试文件、信任矩阵、归档/SBOM/许可证/漏洞策略、源码白名单、离线边界、隐私扫描、CI 硬化、验证映射、干净历史证明。任何意外都会终止链条。 |
| **机器可读的声明账本** | `verification-map.yaml` 把 34 项能力声明绑定到可观察的原因码。声明的主体一旦变化,其证明必须重跑——过度声明是构建失败,不是文风问题。 |
| **双宿主 Agent App 适配器** | Codex 与 Claude Code 是 V1 官方支持的两个宿主,共享逐字节相同的宿主中立机制,并有跨宿主一致性摘要证明。两个适配器均为**惰性干跑候选**:只生成未安装的模板数据,不声称任何在线宿主支持。 |
...
3. **治理需要可串行化的叙事。**单写者链给出决策的线性、防篡改*顺序*;把每个决策绑定到可问责的执行者及其证据,由治理线程的收据在当下承担,并在启用后由工作区链中的执行者留痕扩展承担。一群互相修改共享状态的对等线程,既无顺序也无绑定。
...
- **约 29 个测试文件**,覆盖引擎、知识核心、工件生命周期、配置、人格、上下文路由、双适配器、交换、兼容、需求账本、发布候选、隐私边界、证明工件生成器与信任矩阵。
- `verification-map.yaml` 中 **34 项机器验证声明**。

...
| `verify:act4` / `verify:act9` / `verify:act10` / `verify:act11` / `verify:act12` / `verify:act13` | Codex 惰性安装；独立授权的单一 SessionStart 激活与逐定义漂移；只读 supplied-stream 执行采集；跨宿主验收/敌对矩阵；从敌意 cwd 证明 Codex/Claude 命令只执行准入绝对根路径；以及由精确输出授权和钉扎宿主观测门控的承载 authority 激活回执。 |

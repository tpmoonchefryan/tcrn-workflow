...
- **激活目前还没有面向用户的命令路径**。上述步骤由证据装具驱动，而它是以程序方式供给受治理宿主上下文的。发行的 CLI 不供给，因此从 shell 直接跑 `claude-adapter-install` 会停在 `ADAPTER_HOST_REQUIRED`。能力与其收据都是真实的，缺的是操作者入口。后续版本会补上。
...
- **规模**。一个工作区在**低千位**事件量上就会变得可感知地慢，单条命令跨过一秒约在 **6,600** 事件（基于 Apple M3 上的实测外推；`docs/verification/2026-07-20-event-chain-ceiling-samples.json`）。它影响读取不亚于写入。请**按项目或倡议切分工作区**，而不要让一整个组织跑在一条链上。

...
| **一份机器能读的声明账本** | `verification-map.yaml` 把 69 条声明——13 条 framework-hygiene、13 条 inertness-proof、43 条 runtime-capability——绑定到可观测的 reason code。一条声明的主语变了，它的证明就必须重跑。 |
...
- **69 条机器验证的声明**在 `verification-map.yaml` 中——13 条 framework-hygiene、13 条 inertness-proof、43 条 runtime-capability。上方的声明徽章每次运行都会被解析并与账本比对。

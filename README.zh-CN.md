...
`状态: 0.1.0-rc.4(预发布候选)` · `许可证: Apache-2.0` · `Node 24.16.0` · `pnpm 11.3.0` · `已验证声明: 64`
...
| **机器可读的声明账本** | `verification-map.yaml` 把 64 项能力声明绑定到可观察的原因码。声明的主体一旦变化,其证明必须重跑——过度声明是构建失败,不是文风问题。 |
...
- `verification-map.yaml` 中 **64 项机器验证声明**。

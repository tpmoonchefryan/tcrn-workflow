...
- **本版本之前已有十个获接受的发布，当前是 `0.9.0`。** `0.2.0` 让门身份成真，`0.3.0` 增加咨询式范围，`0.3.2` 打开 `Incident` 创建路径并扩充知识库策展余量，`0.4.0` 增加后台资源残余治理，`0.5.0` 增加 sprint / 发布列车，`0.6.0` 发布受治理的双宿主操作者权威、MCP、激活、observe / execution 收据与 conference provenance，`0.7.0` 把 `externalKey` 放进 `work-list` 摘要并新增分页的 `conference-position-list` 与 `conference-minutes-list`，`0.8.0` 再增加分页的 `event-list`，`0.9.0` 增加受治迁置动词族——移动工作区的绑定，而不移动它这条链的任何一个字节。每个获接受版本都是不可变标签并附可复现产物；`CHANGELOG.md` 记着完整账本。

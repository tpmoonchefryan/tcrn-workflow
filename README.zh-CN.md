...
| **两个宿主，同一份真相** | Codex 与 Claude Code 适配器共享宿主中立的 authority 与回执机制。两者默认都保持惰性，也都有窄化的 fail-open SessionStart 激活；Codex 额外记录逐定义审批边界，并明确不声称宿主那个不透明的信任哈希会被导出。 |
...
底层是冻结的协议，其上是文件原生引擎，再上是能力层，最上是宿主适配器——在一个获得明确批准的激活阶梯之前保持惰性。协议是只追加的：`work-model-v1` 已冻结，每个扩展都自行注册，不触碰已接受的 schema。
...
### Claude Code 的历史 live 收据证明了什么

它证明旧的相对路径定义曾让真实 Claude Code 会话收到一份**仅限 Workflow 的权威边界摘要**，除此之外什么都没有。那份历史摘要限制的是 Workflow 层面的写入；它并没有把主线程变成只读。当前绝对根路径命令改变了精确定义字节，因此旧收据仅属历史证据，本次变更不声称当前 Claude live 激活。
...
被注入的 v2 摘要不绑定任何 Core Reference persona，不会把主线程变成只读，
它限制的只是 Workflow authority，而不是用户对普通仓库工作的显式授权。
...
| `verify:act4` / `verify:act9` / `verify:act10` / `verify:act11` / `verify:act12` / `verify:act13` | Codex 惰性安装；独立授权的 persona-free SessionStart 激活，含逐定义漂移与尚待给出的当前 live 审批；一个只读采集器，外加一次精确的 live App Server 流/回读回执比对；跨宿主验收/敌对矩阵；从敌意 cwd 证明 Codex/Claude 命令只指名准入的绝对 handler 路径，并实测披露解释器在触发时仍经 `PATH` 解析这一未关闭残留；以及由精确输出授权和钉扎宿主观测门控的承载 authority 激活回执。 |
...
- **git 是完整性见证，不是恢复工具**。在工作区根上建仓并配上文档给出的忽略清单，你会得到第二个见证；真正的恢复走快照清单，因为 git 无法重建各个存储所要求的空目录。
- **绝不在工作区之间拷贝存储文件**。每个存储都绑定它自己工作区的历史。跨工作区移动今天是一个规划面：`exchange-plan`、`exchange-dry-run` 与 `exchange-validate` 存在，执行动词不存在。
...
- **完整性不依赖驱动模型的能力，进展才依赖**。失败即关闭把弱驱动的每一次越界都变成一次拒绝，链不可能被弄脏——低于基线的智能体只会在 reason code 上空转，而不是损坏任何东西。真正随模型能力缩放的是：在这套纪律下取得进展、听从注入的权威摘要（已证明它到达，从未主张它被听从）、以及被记录内容的质量——形状合法的垃圾会被如实保存，因为账本证明的是谁说了什么，不是说得对不对。
...
- 当前获接受的发布是 `0.14.0`；它把扩展视图改为定长校验摘要、在事件写入前拒绝超预算的视图投影，并停止把「事实已落、仅派生视图未写成」的写入报告为被拒绝。发布仍由独立治理流程决定。

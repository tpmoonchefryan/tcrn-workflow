...
[为什么做这个项目](#为什么做这个项目) · [这是否适合你](#这是否适合你) · [你会得到什么](#你会得到什么) · [快速开始](#快速开始) · [直白的回答](#直白的回答) · [许可证](#许可证)
...
| | |
| --- | --- |
| ✅ **适合，如果** | 你让智能体处理有后果的工作——生产代码、受监管或需审计的交付、没人记得清是谁做了决定的多智能体交接。你想要的是评审者能*查验*的产物，而不是他们只能*相信*的一段对话记录。你还希望一切留在自己机器上：无数据库、无守护进程、无网络、无遥测。 |
...
| **可以撤销的激活** | 三个显式、逐字节可逆的步骤，把惰性的 Claude Code 包变成一次实时受治理会话——而会话钩子中的任何错误都会干净退出，回落为普通的 Claude Code。惰性包从不命名或写入 `~/.claude` 下的任何东西。 |
| **能自我证明的备份** | 快照会产出确定性的逐文件清单；运行手册可以完成"快照 → 清空 → 恢复"的逐字节往返，而真正要紧的两种失败模式（部分恢复、异地恢复）都失败即关闭。 |
| **两个宿主，同一份真相** | Codex 与 Claude Code 适配器共享逐字节一致的宿主中立机制，并有一个已证明的跨宿主一致性摘要。两者默认都是**惰性干跑候选**——只生成未安装的模板数据。 |
...
底层是冻结的协议，其上是文件原生引擎，再上是能力层，最上是惰性的宿主适配器。协议是只追加的：`work-model-v1` 已冻结，每个扩展都自行注册，不触碰已接受的 schema。
...
因为在受治理的发布路径接受它之前就宣称支持实时宿主，本身就是一次过度声明——正是这个框架存在的意义所要防止的失败模式。适配器生成的是确定性的、未安装的模板包（已证明逐字节精确，包括一段逐字节可逆、从不覆盖用户内容的 `.claude/settings.json` 钩子片段）。激活是一个独立的、带门的决定。
...
- `0.1.0-rc.6` 是一个**预发布候选**。公共 API 尚未稳定。
- 两个宿主适配器都是惰性干跑候选；**不声称支持实时的 Codex 或 Claude Code**。

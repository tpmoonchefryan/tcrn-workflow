...
```verbatim:node scripts/verbatim-evidence-proof.mjs inc153-host-role-scan
{
  ...
  "exitCode": 0,
  "matches": [
    "293:  const roleValue = (role) => typeof role === \"string\" ? role : role?.value;",
    "368:  const activePlan = (host) => { const entry = planSettingForHost(host); return state.execution?.plans?.find((plan) => plan.host === host && plan.name === entry?.currentValue); };",
    "370:    const values = entry.key.includes(\"SubagentPlan\") ? [\"\", ...(state.execution?.plans || []).filter((plan) => plan.host === hostFromPlanKey(entry.key)).map((plan) => plan.name)] : [...(entry.allowedValues || [])];",
    "404:      const hostPlans = plans.filter((plan) => plan.host === host);"
  ]
}
```

...
```verbatim:node scripts/verbatim-evidence-proof.mjs inc153-host-role-scan
{
  ...
  "exitCode": 0,
  "matches": [
    "3463:  const roleValue = (role) => typeof role === \"string\" ? role : role?.value;",
    "3538:  const activePlan = (host) => { const entry = planSettingForHost(host); return state.execution?.plans?.find((plan) => plan.host === host && plan.name === entry?.currentValue); };",
    "3540:    const values = entry.key.includes(\"SubagentPlan\") ? [\"\", ...(state.execution?.plans || []).filter((plan) => plan.host === hostFromPlanKey(entry.key)).map((plan) => plan.name)] : [...(entry.allowedValues || [])];",
    "3574:      const hostPlans = plans.filter((plan) => plan.host === host);"
  ]
}
```

...
- `coverage-baseline.json` 保留既有计数，补入本批新增的
  46 个覆盖面文件，共 136 项；每项带 `testNames`，name-based waiver 不再是
  结构性死分支。

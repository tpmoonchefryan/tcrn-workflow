...
| `WORKSPACE_VIEW_BUDGET_EXCEEDED` | Not retriable unchanged | The workspace's projected views would exceed the canonical ceiling, measured **before** the event segment was written. Nothing was appended: the version and head hash are unchanged, and re-issuing the same command produces the same refusal. The message names the view and the measured size. This is a capacity fact about the workspace, not a defect in the command — the remedy is a workspace-level one (see the record-count and view budgets reported by `status`), not a retry. |
| `WORKSPACE_VIEW_UNWRITTEN` | Informational — never a command's reason code | Appears only as `viewFailureReasonCode` on an otherwise successful receipt, alongside `viewProjection: "unwritten"` and `remedy: "recover"`. The event **is** committed and the reported version and head hash are the real post-append head; only the derived view could not be persisted. Do not retry the mutation — retrying appends a second event for a fact that already landed, which is precisely the defect this field exists to prevent. Acquire the lease and run `recover`. Until then, view-verifying reads raise `WORKSPACE_VIEW_STALE` as usual. |

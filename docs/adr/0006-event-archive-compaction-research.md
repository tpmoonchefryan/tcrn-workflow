...
| --- | ---: | --- |
| Chain version / event count | 5,629 / 10,000 | 4,371 event slots remain; live-record count is not the cap counter |
| Materialized control-tree bytes | 17,480,707 | Snapshot-manifest sum, including event, knowledge, snapshot, and view files |
...
| Estimated control bytes after move | 3,724,062 | Retained control bytes plus a small head-root record |
| Current `status` timing | median 618.4 ms; p95 651.6 ms | 20 serial CLI observations after one warm-up |
| Simulated tail load | median 1.4 ms; p95 1.7 ms | In-process digest over 160 proportionally selected frozen records; not a V2 status measurement |
...
`51b9ba5209a2ecebe014d7eb1405e8cf227fcd26f1af4cf586c1517d4a037aad`; the live
...
from 5,629 to 5,630 events and from 4,371 to 4,370 headroom for one update.
...
```text
corepack pnpm run research:compaction -- --workspace <cross-project-workspace> --runs 20 --at 2026-09-10T22:25:00Z
```

...
| **A claim ledger a machine can read** | `verification-map.yaml` binds 70 claims — 13 framework-hygiene, 13 inertness-proof, 44 runtime-capability — to observable reason codes. If a claim's subject changes, its proof must re-run. |
...
- **70 machine-verified claims** in `verification-map.yaml` — 13 framework-hygiene, 13 inertness-proof, 44 runtime-capability. The claims badge above is parsed and compared against the ledger on every run.

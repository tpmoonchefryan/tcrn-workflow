...
`Verified claims: 9 (hygiene 8 · inertness 0 · runtime 1)`
...
> [!TIP]
> **不必相信这份 README**。装上之后跑一条命令，它会把自己的 9 条声称逐条证明给你看，全程离线。
...
| 你想确认的 | ✗ 你现在有的 | ✓ 装上之后你有的 |
| :--- | :--- | :--- |
| **测试真的跑过吗** | 聊天窗口里的一行字 | `pnpm verify:p1`——16 道门按序跑完，任何一处意外就停下 |
...
这个标准覆盖全部 **9 条声称**。每一条都在 `verification-map.yaml` 里绑定一个稳定的原因码、一条能离线跑的证明、一条红腿——写明什么改动会让它变红——以及它挂在哪一类门上（`requirement`）或哪一次事故（`incident`）。9 条，无一例外。
...
<details>
<summary><b>9 条判据是怎么分布的</b></summary>

<br>

| 类别 | 条数 | 管什么 |
| :--- | ---: | :--- |
| `framework-hygiene` | 8 | 框架自身的卫生：干净历史、许可与漏洞策略、离线边界、隐私扫描、隔离检出、发行候选 |
| `inertness-proof` | 0 | 空：这一类的判据都挂在 STORY-359 撤掉的 `verify:*` 名字上；它们点名的测试仍在 `pnpm test` 里跑 |
| `runtime-capability` | 1 | 外部键片段在两个检索索引中保持可验证的前缀命中 |
...
| **一条命令跑完 16 道门** | `pnpm verify:p1` 依次跑格式、lint、类型、构建、124 个测试文件（含覆盖注册表与存活模块覆盖检查）、门户校验、源码白名单、源码归档、兄弟仓依赖、离线边界、工具链治理（许可、生命周期、漏洞策略、干净历史）、隐私扫描、判据账本、证明预算、文档链接、检索评测。 |
| **9 条机器可读的判据** | 全部框架卫生或运行时能力，全部带红腿，全部绑定可观测的原因码，并各自挂在十一类门中的一类上。 |
...
测试腿的文件数对应 `gate-test.log` 的 `tests` 数组长度。`scripts/policy/coverage-baseline.json` 的 `files` 共登记 127 个文件，其中 124 个由测试腿运行，另外三个 `portal/tests/*` 文件由门户腿运行。README 的 P1 门数与枚举由 `tests/p1-roster.test.mjs` 对照 `scripts/p1-sequence.mjs` 的实际腿表检查。
...
当前候选版本是 **1.1.2**，尚未发布。每个受理版本都是一个不可变的标签加一套可复现的产物，`CHANGELOG.md` 是完整账本。

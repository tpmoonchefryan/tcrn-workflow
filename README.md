...
inert rung. No PreToolUse/approval enforcement or active App Server Controller is
claimed.
...
| `verify:act4` / `verify:act9` / `verify:act10` / `verify:act11` / `verify:act12` / `verify:act13` | Codex inert install; independently authorized single SessionStart activation and exact-definition drift; read-only supplied-stream execution collection; cross-host acceptance/hostile matrix; hostile-cwd proof that Codex/Claude commands execute only the admitted absolute root; and authority-bearing activation receipts gated by an exact output grant and pinned host observation. |

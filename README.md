...
| **Machine-readable claim ledger** | `verification-map.yaml` binds 57 claims — 12 framework-hygiene, 13 inertness-proof, 32 runtime-capability — to observable reason codes. If a claim's subject changes, its proof must re-run — overclaiming is a build failure, not a style issue. |

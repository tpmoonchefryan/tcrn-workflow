...
- **Nine accepted releases preceded this one, and this is `0.8.0`.** `0.2.0` made gate identity real, `0.3.0` added advisory scope, `0.3.2` opened the `Incident` create path and knowledge-store headroom, `0.4.0` added background-resource residue governance, `0.5.0` added sprint / release trains, `0.6.0` shipped governed dual-host operator authority, MCP, activation, observe/execution receipts and conference provenance, `0.7.0` put `externalKey` into `work-list` summaries and added the paginated `conference-position-list` and `conference-minutes-list`, and `0.8.0` adds the paginated `event-list`. Each accepted version is an immutable tag with a reproducible artifact set; `CHANGELOG.md` carries the full ledger.

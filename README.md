...
| **Guards that prove they still bite** | `pnpm guard-check` mutates each registered guard out of the source and requires its named test to go red — 22 guards, verified before every push. A protection that nothing would notice losing is not a protection. |
...
- **22 registered guards**, each proven to still bite by mutating it out and watching its test go red.
...
- **Privacy gate** over all 333 allowlisted source files (an exact-match list — one file added or removed fails the gate), every reachable git object, and the release archive.
...
- **Seven accepted releases preceded this one, and this is `0.6.0`.** `0.2.0` made gate identity real, `0.3.0` added advisory scope, `0.3.2` opened the `Incident` create path and knowledge-store headroom, `0.4.0` added background-resource residue governance, `0.5.0` added sprint / release trains, and `0.6.0` ships governed dual-host operator authority, MCP, activation, observe/execution receipts and conference provenance. Each accepted version is an immutable tag with a reproducible artifact set; `CHANGELOG.md` carries the full ledger.

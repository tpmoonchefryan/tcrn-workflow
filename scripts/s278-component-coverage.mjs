...
const baselineText = execFileSync("git", ["show", "HEAD:portal/index.html"], { encoding: "utf8" });
const baselineRoots = rootNames(portalStyle(baselineText));
...
const classRows = baselineRoots.map((className) => ({
  className,
  baselinePresent: true,
  currentPresent: currentRoots.includes(className),
  snapshotPresent: snapshotRoots.includes(className),
  status: currentRoots.includes(className) ? "covered" : "missing",
}));

const missingComponents = componentRows.filter((row) => row.status !== "covered");
const missingPortalClasses = classRows.filter((row) => row.status !== "covered");
const renamedOrRemoved = removedBySnapshotDiff.length > 0;
const ok = baselineRoots.length === 51
...
process.stdout.write(`${JSON.stringify({
  ...
  portalOwnedClassBaseline: {
    expectedCount: 51,
    actualCount: baselineRoots.length,
    rows: classRows,
    snapshotDiff: {
      removedOrRenamed: removedBySnapshotDiff,
      added: addedBySnapshotDiff,
      method: "set difference between the S253 baseline snapshot and the current class-root set",
    },
  },
  returnedComponents: {
    expectedCount: 9,
  ...
  decisionSummary: {
    platformLayer: "Keep the 51 S253 portal class roots stable; consume the eight named S254 return components from the DS snapshot; use the existing readback panel for the ninth slot and add no new component.",

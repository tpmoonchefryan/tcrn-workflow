...
| **Un registre de revendications lisible par machine** | `verification-map.yaml` lie 71 revendications — 13 framework-hygiene, 13 inertness-proof, 45 runtime-capability — à des reason codes observables. Si le sujet d'une revendication change, sa preuve doit être rejouée. |
...
- **71 revendications vérifiées par la machine** dans `verification-map.yaml` — 13 framework-hygiene, 13 inertness-proof, 45 runtime-capability. Le badge de revendications ci-dessus est analysé et confronté au registre à chaque exécution.

...
| **Un registre de revendications lisible par machine** | `verification-map.yaml` lie 69 revendications — 13 framework-hygiene, 13 inertness-proof, 43 runtime-capability — à des reason codes observables. Si le sujet d'une revendication change, sa preuve doit être rejouée. |
...
- **69 revendications vérifiées par la machine** dans `verification-map.yaml` — 13 framework-hygiene, 13 inertness-proof, 43 runtime-capability. Le badge de revendications ci-dessus est analysé et confronté au registre à chaque exécution.

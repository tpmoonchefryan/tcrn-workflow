...
test("INC-269 an annotation that moves no advisory field is refused before it is written", async (context) => {
  const { workspace, lease, id, version } = await incidentFixture(context, "INC269-GUARD");
  assert.equal(version, 2);

  await assert.rejects(
    () => annotateWork(workspace, lease, {
      expectedVersion: version, occurredAt: instant(4), id, title: "T", labels: ["a", "b"],
    }),
    (error) => error?.reasonCode === "WORKSPACE_INPUT_INVALID"
      && error.message === `work ${id} annotation changed no advisory field`,
  );
  ...
  // The criterion is "an advisory value moved", not "an advisory flag was passed": re-sending
  // the same scope alongside a new title changes no advisory field and is refused identically.
  // This is the reducer's predicate, so the verb and replay cannot disagree about it.
  const annotated = await annotateWork(workspace, lease, {
    expectedVersion: version, occurredAt: instant(5), id, scope: "INC-269 advisory guard probe",
  });
  await assert.rejects(
    () => annotateWork(workspace, lease, {
...
test("INC-269 title and labels still land when the same annotation moves an advisory field", async (context) => {
